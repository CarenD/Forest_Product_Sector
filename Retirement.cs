                            ...
                            { 
                                ...
                                if (Poollist.Contains(pcode))  //(1000, 1001, 1003, 1009, 1010)
                                {
                                    ...
                                    for (jyr = 1; jyr <= (YearsAfter + maxyears - so.SpecialOutputYear); jyr++)
                                    {
                                        ...
                                        /* try moving this code with 1522 logic

                                        //Anaerobic (1500): The portion of LandfillWood and DumpPaper that is decayed goes to
...
*/
                                        ...
                                        //The portion of LandfillWood and DumpPaper that is decayed goes to 50% E_CO2 (2006) and 50% potCH4 (2008).
                                        ...
                                        else if (pp.RespirationType == 1500)
                                        {
                                            ...
                                            {  
                                                ...
                                                if ((lfg.RespirationType == pp.RespirationType) && RespirationMatchFound == false)
                                                {
                                                    ...
                                                    // find which lfg respiration code bit to use: 1500 or 1522
                                                    // if (lfg.RespirationType == 1500)
                                                    if (lfg.PropDecay == 0 && lfg.Efficiency == 0)
                                                    {
                                                        newCO2 = potCH4 * lfg.Oxidation;                    // CH4->CO2 via oxidation
                                                        eCH4   = potCH4 - newCO2;                           // remove from potCH4 pool
                                                        potCH4 = 0;                                         // because it all either became CO2 or CH4
                                                    }
                                                    // else if (lfg.RespirationType == 1522)
                                                    else
                                                    {
                                                        newCO2  = potCH4 * lfg.PropDecay * lfg.Efficiency;  // CH4->CO2 via decay and oxidation;
                                                        potCH4 -= newCO2;                                   // remove from potCH4 pool   
